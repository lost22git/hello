...
(define (comp f . gs)
  ...
  (letrec ([rf (lambda (state cur)
                 (lambda (x) (state (cur x))))])

    (fold-left rf f gs)))

;; compose one function
(define comp-one (comp string-upcase))
...
(define (juxt f . gs)
  ...
  (lambda xs
    (map (lambda (f) (apply f xs)) (cons f gs))))
...
(-> (string-upcase "abc") (string=? "ABC") assert)
