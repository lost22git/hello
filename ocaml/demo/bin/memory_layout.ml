...
let instance_payload_words v =
  Obj.size (Obj.repr v)

(** get bytes count of payload of instance on heap *)
let instance_payload_bytes v = 
  (Sys.word_size / 8) * instance_payload_words v

let instance_words v = 
  Obj.reachable_words (Obj.repr v)

let instance_bytes v =
  (Sys.word_size / 8) * instance_words v
